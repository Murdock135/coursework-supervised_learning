...
if __name__ == '__main__':
    ...
    # Fit GMM
    gmm = GaussianMixtureModel(3, 100)
    gmm.fit(X_train)
    centers = gmm.final_parameters['mean_vectors']
    train_labels, train_responsibility_matrix = gmm.predict(X_train)
    test_labels, test_responsibility_matrix = gmm.predict(X_test)

    print("Labels for training set:")
    print(train_labels)
    print("Labels for testing set:")
    print(test_labels)

    fig = plot_data_overview(X_train, train_labels, train_responsibility_matrix, centers=centers, is_train=True)
    fig = plot_data_overview(X_test, test_labels, test_responsibility_matrix, centers=centers, is_train=False)
    plt.show()
